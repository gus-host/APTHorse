	...
	public class TerrainManager
	{
		...
		public Terrain GetTerrain(Vector3 worldPosition)
        {
			...
			foreach (Terrain terrain in terrainGroup)
			{
				...
				// world position
				// instead of heightmap size we use the alphamap resolution
				float x = ((worldPosition.x - terrainWorldPosition.x) / terrain.terrainData.size.x) * terrain.terrainData.heightmapResolution;
				float z = ((worldPosition.z - terrainWorldPosition.z) / terrain.terrainData.size.z) * terrain.terrainData.heightmapResolution;
		...
		private bool ConvertToSplatMapCoordinate(Terrain terrain, Vector3 worldPosition, ref Vector3 splatPosition)
		{
			...
			// instead of heightmap size we use the alphamap resolution
			splatPosition.x = ((worldPosition.x - terrainWorldPosition.x) / terrain.terrainData.size.x) * terrain.terrainData.heightmapResolution;
			splatPosition.z = ((worldPosition.z - terrainWorldPosition.z) / terrain.terrainData.size.z) * terrain.terrainData.heightmapResolution;
